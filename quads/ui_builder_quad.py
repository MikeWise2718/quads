...
class UIBuilder:
    ...
    def LoadSettings(self):
        ...
        self._base_scenario_action_list = []
        self._scenario_action_list = []
        self._base_robot_action_list = []
        self._robot_action_list = []

        # set the default action in the UI
        action = get_setting("p_action", self._action)
        defaction = "--none--"
        if len(self._scenario_action_list) > 0:
            defaction = self._scenario_action_list[0]

        self._action = action if action in self._scenario_action_list else defaction
...
#                         ScenarioBase.get_scenario_desc(self._scenario_name),
    ...
    def load_scenario_action_vstack(self):
        ...
        with self._scenario_action_vstack:
            if self._cur_scenario is None:
                self._scenario_action_list = ["--none--"]
            else:
